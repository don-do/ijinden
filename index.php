...
//POST送信されていた場合
if(!empty($_POST)){
  $studyFlg = (!empty($_POST['study'])) ? true : false;
  $startFlg = (!empty($_POST['start'])) ? true : false;
  error_log('POST送信されました。');
  ...
  if($startFlg){
    ...
    if($studyFlg){
      ...
      // 偉人から学び取る
      History::set('<br>'.$_SESSION['student']->getName().'が、学び取る！！偉人にヒット！');
      $_SESSION['student']->study($_SESSION['great']);
      $_SESSION['great']->saySerif();

      // 偉人が問題を出す
      History::set('<br>'.$_SESSION['great']->getName().'が問題を出してきた！<br>難問だ！学ぶ者の集中力が減る！！');
      $_SESSION['great']->study($_SESSION['student']);
      $_SESSION['student']->saySerif();
      ...
      if($_SESSION['student']->getConcentration() <= 0){
        ...
        if($_SESSION['great']->getConcentration() <= 0){
          History::set('<br>おや！？偉人が負けを認めた！！<br>'.$_SESSION['great']->getName().'は、降参したよ！<br><br>');
          createGreat();
          $_SESSION['understandingCount'] = $_SESSION['understandingCount']+1;
        }
      }
    }else{ //逃げるを押した場合。別の偉人が現れる
      History::set('分からん！逃げろ！<br><br>');
      createGreat();
    }
  }
...
<body>
  <div class="wrapper">
    <?php if(empty($_SESSION)){ ?>
      <div class="start">
        <h2 class="start__title">偉 人 伝</h2>
          ...
          <div class="start__text">
            <p>英・数・国・理・社・・<br>いろんな学問を学び取れ！<br>何人の偉人から学び取ることができるか！？</p>
          </div>
      </div>
    <?php }else{ ?>
      <section class="game">
        <h1 class="game__title">偉人を学び倒せ！！</h1>
          <div class="game__container">
            <div class="great">
              <div class="great__table">
                <div class="great__img">
                  <img src="<?php echo $_SESSION['great']->getImg(); ?>">
                </div>
              </div>
              <div class="great__info">
                <p class="game__panel--name"><?php echo $_SESSION['great']->getName(); ?></p>
                <div class="game__panel--info">
                  <img src="img/icon_concentration.png" class="game__panel--concentration">偉人の<br>集中力：<?php echo $_SESSION['great']->getConcentration(); ?> / <?php echo $_SESSION['great']->getMaxconcentration(); ?></div>
              </div>
            </div>
            <div class="studentview">
              <div class="studentview__table">
                <div class="studentview__serif"><?php echo $_SESSION['great']->saySerif(); ?></div>
              </div>
              <div>
                <form method="post" class="studentview__btn">
                  <input class="post--esc" type="submit" name="escape" value="▶逃げる">
                  <input class="post--std" type="submit" name="study" value="▶学び取る">
                </form>
              </div>
              <div class="studentview__info">
                <img src="img/icon_concentration.png" class="studentview__info--concentration">学ぶ者の集中力<br>残り：<?php echo $_SESSION['student']->getConcentration(); ?></div>
            </div>
          </div>
      </section>
      <section class="log">
        <div class="log__container">
          <div class="log__text js-auto-scroll" id="history">
            <p><?php echo (!empty($_SESSION['history'])) ? $_SESSION['history'] : ''; ?></p>
          </div>
          <div class="log__right">
            <div class="log__victory">
              <div class="log__victory--wrap">
                <div class="log__victory--inner">
                  <img src="img/icon_victory.png" alt=""><br>
                  <p>倒した<br>偉人の数：<?php echo $_SESSION['understandingCount']; ?></p>
                </div>
              </div>
            </div>
            <form method="post">
              <input class="post--rst" type="submit" name="start" value="▶もう一度最初から">
            </form>
          </div>
        </div>
      </section>
  ...
  <footer class="footer">
    <p>Copyright © <a href="https://don-d.com">don-d.com</a>. All Rights Reserved</p>
  </footer>

  <script src="js/jquery-3.4.1.min.js"></script>
  <script>
    $(function() {
      $('.js-auto-scroll').animate({scrollTop: $('.js-auto-scroll')[0].scrollHeight}, 'fast');
    })
  </script>
</body>
</html>
